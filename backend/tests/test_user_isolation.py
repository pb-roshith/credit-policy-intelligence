...
Only randomly named test-user schemas are created and removed.
"""
import unittest
...
from app.database import db_connection, shared_policy_connection, user_schema
...
class UserIsolationTest(unittest.TestCase):
    def setUp(self):
        app = FastAPI()
        for module in (requests, policies, controls, credit_requests, exposure_history, policy_pdfs):
        ...
        with db_connection() as connection:
            for session in self.sessions:
                ...
                connection.execute(sql.SQL("DROP SCHEMA IF EXISTS {} CASCADE").format(
                    sql.Identifier(user_schema(session["user"]["user_id"]))
                ))
